...
export async function updateSynapsePoints(amount, topicKey, taskKey) {
    try {
        ...
        const currentSB = userData.synapseBucks || 0;
        ...
        const newSB = currentSB + (isCompleted ? -amount : amount);
...
const updatedTask = isCompleted
    ? { completed: false, timestamp: null } // Mark as not completed but retain the entry
...
const updatedTopicTasks = {
    ...
    [taskKey]: updatedTask,
        ...
        return { newSP, newSB, newTier, isCompleted: !isCompleted }; // Return toggled state
    } catch (error) {
...
export async function setupActivity(elementId, taskKey, points, topicKey) {
    ...
    try {
        ...
        element.addEventListener('click', async () => {
            try {
                const { newSP, newSB, newTier, isCompleted: toggledCompleted } = await updateSynapsePoints(
                    points,
                    topicKey,
                    taskKey
                );
                ...
                showPopup(`Task ${toggledCompleted ? 'completed' : 'undone'}! New SP: ${newSP}, New SB: ${newSB}, Tier: ${newTier}`);
            } catch (error) {
